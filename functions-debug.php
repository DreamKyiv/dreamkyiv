<?php
if ( !defined('SAVEQUERIES') && isset($_GET['ddd123']) && $_GET['ddd123'] == 'sql' )
...
function dump_stops($in = null) {
    ...
    foreach ( $sem_stops as $where => $stop )
        $stops .= "$where: $stop\n";
    dump("\n" . trim($stops) . "\n");
    if ( defined('SAVEQUERIES') && $_GET['ddd123'] == 'sql' ) {
        global $wpdb;
...
function dump_phpinfo() {
    if ( isset($_GET['ddd123']) && $_GET['ddd123'] == 'phpinfo' ) {
        phpinfo();
        die;
    }
} # dump_phpinfo()
